...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    email = models.EmailField(
        verbose_name="Adresse email", blank=False, max_length=100, unique=True
    )
    username = None
...
class Events(models.Model):
    ...
    customer = models.ForeignKey(
        to=Customer, on_delete=models.CASCADE, related_name="events"
    )
    date_created = models.DateTimeField(
        verbose_name="Date de création", auto_now_add=True
    )
    ...
    support_member = models.ForeignKey(
        to=User, on_delete=models.SET_NULL, related_name="events", null=True
    )
    event_status = models.CharField(
        choices=EventStatus.choices, default=EventStatus.ON_GOING
    )
...
class Contract(models.Model):
    sales_member = models.ForeignKey(
        to=User, on_delete=models.SET_NULL, related_name="contracts", null=True
    )
    customer = models.ForeignKey(
        to=Customer, on_delete=models.CASCADE, related_name="contracts"
    )
